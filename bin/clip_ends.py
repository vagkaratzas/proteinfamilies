...
def calculate_trim_positions(sequence_matrix, gap_threshold):
    ...
    end_position = len(column_sums_percentage) - np.argmax(column_sums_percentage[::-1] > gap_threshold) - 1

    return start_position, end_position

def write_trimmed_sequences(sequence_matrix_trimmed, original_sequence_names, out_fasta):
    trimmed_records = []
...
def trim_msa(alignment, gap_threshold, out_fasta):
    ...
    start_position, end_position = calculate_trim_positions(sequence_matrix, gap_threshold)
